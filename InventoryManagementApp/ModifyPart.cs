...
namespace InventoryManagementApp
{
    public partial class ModifyPart : Form
    {
        ...
        public ModifyPart(int modifyPartID)
        {
            InitializeComponent();

            initializeData(modifyPartID);
        }

        private void initializeData(int modifyPartID)
        {
            this.txtPartID.Text = modifyPartID + "";
            Part part = Inventory.lookupPart(modifyPartID);
            txtPartName.Text = part.Name;
            txtPartInventory.Text = "" + part.InStock;
            txtPartPriceCost.Text = "" + part.Price;
            txtPartMax.Text = "" + part.Max;
            txtPartMin.Text = "" + part.Min;

            if (part is Inhouse)
            {
                txtPartMachineIDCompanyName.Text = ((Inhouse)part).MachineID + "";
                radioButtonInhouse.Checked = true;
            }
            else
            {
                txtPartMachineIDCompanyName.Text = ((Outsourced)part).CompanyName;
        ...
        private Boolean missingFields()
        {
            ...
            if (txtPartMachineIDCompanyName.Text.Length == 0)
            {
                ...
                MessageBox.Show("Please enter a value to Part MachineID / CompanyName", "Error!!");
                return true;
